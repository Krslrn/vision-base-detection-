...
class VisionBasedCalculator:
    ...
    def draw_feedback(image, distance, angle, status):
        ...
        lines = [
            ...
            f"Angle: {int(angle)}deg - {'OK' if status['angle'] == 'OK' else 'Adjust'}",
            f"Feedback: {status['feedback']}",
        ]
        if status['feedback'] == "Error":
            lines.append("Please fix your positioning to the acceptable range.")
    ...
    def calculate_angle_and_distance(self, pixels, distances):
        ...
        while cap.isOpened():
            ...
            if not results.detections:
                ...
                if corrected_distance < 40 or corrected_distance > 74 or abs(vertical_angle) < 15 or abs(vertical_angle) > 30:
                    feedback = "Error"
                else:
                    feedback = "OK"
